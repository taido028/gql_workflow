...
async def authorization_add_user(
    self, info: strawberry.types.Info, authorization: AuthorizationAddUserGQLModel
) -> Optional["AuthorizationResultGQLModel"]:
    loader = getLoaders(info).authorizationusers
    existing = await loader.filter_by(
        authorization_id=authorization.authorization_id, user_id=authorization.user_id
    )
